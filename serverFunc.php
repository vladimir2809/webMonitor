...
{
    $_SESSION['data']=$_POST;
    if (mb_strlen($_POST['nameUser'])<3 || mb_strlen($_POST['nameUser'])>100)// проверим имя на длину
    {
        $_SESSION['errorReg']="Имя должно быть не менее 3 и не более 100 сисмволов";
        header("Location: "."registration.php");
        exit;
    }
    if (mb_strlen($_POST['surnameUser'])<3 || mb_strlen($_POST['surnameUser'])>100)// проверим фамилию на длину
    {
        $_SESSION['errorReg']="Фамилия должна быть не менее 3 и не более 100 символов.";
        header("Location: "."registration.php");
        exit;
    }
    if (mb_strlen($_POST['login'])<3 || mb_strlen($_POST['login'])>100)// проверим логин на длину
    {
        $_SESSION['errorReg']="Логин должен быть не менее 3 и не более 100 символов.";
        header("Location: "."registration.php");
        exit;
    }
    if (mb_strlen($_POST['password'])<6)// проверим пароль на длину
    {
    ...
    if ($_POST['checkboxSms']=='on')// если включен ческбокс отправлять смс
    {
        ...
        if (mb_strlen($_POST['telephone'])!=10)// проверяем что телефон введен полностью
        {  
